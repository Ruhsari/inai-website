...
export function EventsSection() {
  ...
  return (
    <section id="events" className="py-24 bg-[#FAFAFA]">
      <Container size="lg">
        ...
        <div className="flex flex-col gap-8">
          {filteredEvents.map((event: EventItem) => (
            <Link 
              href={`/events/${event.id}`} // <-- ССЫЛКА НА СТРАНИЦУ ПОДРОБНОСТЕЙ
              key={event.id}
              className="bg-white rounded-[24px] shadow-[0_8px_30px_rgb(0,0,0,0.06)] overflow-hidden flex flex-col md:flex-row p-4 md:p-6 gap-8 items-center cursor-pointer group hover:shadow-lg transition-all duration-300"
            >
              ...
              <div className="relative w-full md:w-[45%] h-[240px] md:h-[280px] shrink-0 rounded-xl overflow-hidden">
                <Image 
                  ...
                  alt={event.title} 
                  fill 
                  className="object-cover group-hover:scale-105 transition-transform duration-500" // Добавили зум при наведении
                  unoptimized
                />
              </div>
              ...
              <div className="w-full md:w-[55%] flex flex-col justify-center py-4 pr-4">
                ...
                <Title 
                  ...
                  className="text-[#1A235E] font-bold text-2xl md:text-[28px] leading-tight group-hover:text-[#2A347E] transition-colors"
                >
                  {event.title}
                </Title>
                ...
                {/* 3. КНОПКА-ССЫЛКА (Теперь это просто визуальный элемент внутри большой ссылки) */}
